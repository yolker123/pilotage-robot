...
class MainWindow(QMainWindow):
    executeFunction = pyqtSignal(
        object)  # signal permettant de faire executer une fonction dans le thread de la feunetre principal
    ...
    def __init__(self):
        ...
        self.items = [p[1] for p in
                      self.portlist]  # take only the p[1] (the second value) of each p in portlist to the list items[].
        ...
        grid.addWidget(title_speed, 0, 0, 1,
                       5)  # start location from line 0,colone0 occupy the 1st and 5th colones (from 1 to 5)
        ...
        slider.sliderReleased.connect(
            self.valueChanged)  # save value to valuechanged function when release the slider with mouse.
        ...
        self.buttonHomePosition.clicked.connect(
            lambda: self.moveHome())  # Go to function S_Gotohome, it's outside this class, so no self. S_GoToHome() make robort arm to initial position
        ...
        customCubeFieldWidgets = [self.label_x_cube_size, self.label_y_cube_size, self.label_z_cube_size,
                                  self.x_cube_size, self.y_cube_size, self.z_cube_size, self.label_x_cube_points,
                                  self.label_y_cube_points, self.label_z_cube_points, self.x_cube_points,
                                  self.y_cube_points, self.z_cube_points, self.unit_label]
    ...
    def customCube(self):
        ...
        def gui1():
            ...
            dataset = customCube(x, y, z,
                                 float(self.x_cube_size.text()), float(self.y_cube_size.text()),
                                 float(self.z_cube_size.text()),
                                 int(self.x_cube_points.text()), int(self.y_cube_points.text()),
                                 int(self.z_cube_points.text()))
            ready = True
        ...
        Acquire_points(dataset, self.robot, x, y, z, log_dir, self.scope, self.measurementNumber)
    ...
    def valuePort(self):
        ...
        print("port : ",
              port.currentIndex())  # currentIndex() is the number the sender() get from port list (the line you chose with your mouse). 0-->first line or 1--> second line etc.
        COM = self.portlist[port.currentIndex()][
            0]  # if the first line chosen, currentIndex() = 0, so portlist[port.currentIndex][0] =  portlist[0][0]. Set FIRST LINE, first colone of portlist to com
    ...
    def getFigureList(self):
        return [
            ...
            [self.MeasureOscillo, self.nfc, self.emvco, self.customCube, self.MeasureOnePoint, self.customCylindre,
    ...
    def onFigureSelected(self):
        ...
        customCubeFieldWidgets = [self.label_x_cube_size, self.label_y_cube_size, self.label_z_cube_size,
                                  ...
                                  self.label_y_cube_points, self.label_z_cube_points, self.x_cube_points,
                                  self.y_cube_points, self.z_cube_points, self.unit_label]
        customCylindreFieldWidgets = [self.label_radius, self.field_radius, self.label_height, self.field_height,
                                      self.label_z_points, self.field_z_points, self.label_theta_points,
                                      self.field_theta_points, self.unit_label]
        customSemispereFieldWidgets = [self.label_radius, self.field_radius, self.label_z_points, self.field_z_points,
                                       self.label_theta_points, self.field_theta_points, self.unit_label]
    ...
    def udpdateEnable(self):
        ...
        if self.canMeasure and self.canMove and self.getFigureList()[1][
            self.figureSelector.currentIndex()] != self.MeasureOscillo:  # canMove and canMeasure except for manualMeasure
            if self.getFigureList()[1][self.figureSelector.currentIndex()] == self.customCube:
            ...
            self.buttonLaunch.setEnabled(False)
        if self.canMeasure and self.canMove and self.getFigureList()[1][
            self.figureSelector.currentIndex()] == self.MeasureOnePoint:
